...
use gpu_charts_shared::{ChartConfiguration, VisualConfig, Result, Error};
...
impl ConfigValidator {
    ...
    fn validate_visual_config(config: &VisualConfig) -> Result<()> {
        // Validate colors
        for color in [&config.background_color, &config.grid_color, &config.text_color] {
    ...
    fn validate_overlay(overlay: &gpu_charts_shared::OverlayConfig) -> Result<()> {
        ...
        if !supported_types.contains(overlay.overlay_type.as_str()) {
            return Err(Error::InvalidConfiguration(
                format!("Unsupported overlay type: {}", overlay.overlay_type)
            ));
        }
...
impl ConfigurationDiff {
    ...
    pub fn calculate(old: &ChartConfiguration, new: &ChartConfiguration) -> Self {
        Self {
            ...
            data_handles_changed: old.data_handles.len() != new.data_handles.len() ||
                old.data_handles.iter().zip(&new.data_handles).any(|(o, n)| o.id != n.id),
        }
    }
    ...
    pub fn requires_update(&self) -> bool {
        self.visual_changed || self.chart_type_changed || self.overlays_changed || self.data_handles_changed
    }
}