...
export function useWasmChart(options: UseWasmChartOptions): [WasmChartState, WasmChartAPI] {
  ...
  const storeMetricPreset = useAppStore(state => state.ChartStateConfig.metricPreset);
  const storeStartTime = useAppStore(state => state.ChartStateConfig.startTime);
  const storeEndTime = useAppStore(state => state.ChartStateConfig.endTime);
  ...
  // Placeholder for disabled performance monitoring
  const performanceState = {
    isMonitoring: false,
    metrics: { fps: 0, renderLatency: 0 }
  };
  ...
  const performanceAPI = {
    ...
    stop: () => {},
    reset: () => {},
    getMetrics: () => ({ fps: 0, renderLatency: 0 })
  };
        ...
        chart = {
          ...
          get_current_state: async () => {
            return {
              ...
              ChartStateConfig: { metricPreset: storeMetricPreset },
  ...
  }, [canvasId, width, height, enableAutoSync, enablePerformanceMonitoring, performanceAPI, storeSymbol, storeConnected, storeMetricPreset, storeStartTime, storeEndTime, errorAPI]);

  /**
   * Update chart state from store state
   */
  const updateState = useCallback(async (symbol?: string, connected?: boolean): Promise<boolean> => {
    ...
    try {
      console.log('[useWasmChart] Chart state update requested:', {
        ...
        metricPreset: storeMetricPreset,
      ...
      // Use the new update_chart_state method with simplified store state
      if (chartState.chart.update_chart_state) {
        try {
          // Get the current store state and pass simplified version to WASM
          const storeState = useAppStore.getState();
          const storeStateJson = JSON.stringify({
            currentSymbol: currentSymbol,
            preset: storeState.ChartStateConfig.metricPreset,
            startTime: storeState.ChartStateConfig.startTime,
            endTime: storeState.ChartStateConfig.endTime,
          });

          console.log('[useWasmChart] Sending simplified store state to WASM:', storeStateJson);
        ...
        try {
          await chartState.chart.update_state(currentSymbol, 'default', currentConnected);
        } catch (wasmError) {
      ...
      await errorAPI.reportStoreError(
        ...
        'sync',
        {
          storeState: { symbol: currentSymbol, connected: currentConnected },
  ...
  }, [chartState.chart, chartState.isInitialized, storeSymbol, storeConnected, storeMetricPreset, storeStartTime, storeEndTime, errorAPI]);
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(() => {
      if (mountedRef.current && updateStateRef.current && chartState.chart) {
        ...
        updateStateRef.current(storeSymbol, storeConnected);
      }
    }, debounceMs);
  ...
  }, [storeSymbol, storeConnected, storeMetricPreset, storeStartTime, storeEndTime, enableAutoSync, chartState.isInitialized, chartState.chart, debounceMs]);

  // Note: Chart type and candle timeframe are now handled by WASM presets,
  // not by React state, so we removed those effects
