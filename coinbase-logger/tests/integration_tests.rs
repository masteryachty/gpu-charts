...
mod integration_tests {
    ...
    async fn test_message_processing_pipeline() {
        ...
        assert_eq!(parsed.get("product_id").unwrap().as_str().unwrap(), "BTC-USD");
    }
    ...
    async fn test_symbol_distribution_across_connections() {
        ...
        let symbols: Vec<String> = (0..197)
            .map(|i| format!("SYMBOL-{}-USD", i))
            .collect();
    ...
    fn test_configuration_constants() {
        ...
        assert_eq!(coinbase_logger::connection::MAX_RECONNECT_DELAY.as_secs(), 60);
